...
SECTION_KEYWORDS: dict[str, list[str]] = {
    "summary": [
        ...
        "özet", "hakkımda", "hakkında", "profil", "kişisel özet",
    ],
    "experience": [
        "experience", "work experience", "professional experience",
        "employment", "employment history", "work history",
        "career history", "positions held",
        "deneyim", "iş deneyimi", "çalışma geçmişi", "kariyer",
    ],
    "education": [
        "education", "academic background", "academic history",
        "qualifications", "degrees", "schooling",
        "eğitim", "öğrenim", "akademik geçmiş",
    ],
    "skills": [
        "skills", "technical skills", "core competencies",
        "competencies", "technologies", "tools", "proficiencies",
        "key skills", "areas of expertise",
        "yetenekler", "beceriler", "teknolojiler", "yetkinlikler",
        "teknik beceriler",
    ],
    "projects": [
        "projects", "personal projects", "key projects",
        "portfolio", "open source",
        "projeler", "kişisel projeler",
    ],
}

# Turkish word list used for quick language heuristic
TURKISH_WORDS = {
    "ve", "bir", "bu", "da", "de", "için", "ile", "ben", "ama",
    "olan", "gibi", "çok", "ise", "ya", "ya da", "ki", "mı",
    "deneyim", "eğitim", "beceri", "hakkımda", "özet",
    "çalışma", "proje", "yetenek",
}
...
_NS_PUNCT_SPACING    = re.compile(r"([.!?;:,])([A-Za-zÀ-ÖØ-öø-ÿ\u011e\u011f\u015e\u015f\u0130\u0131])")
...
def _detect_page_layout(page, words: list[dict]) -> str:
    ...
    right_words_t = [
        {**w, "x0": w["x0"] - split_x, "x1": w["x1"] - split_x}
...
def _words_to_text(word_list: list[dict], y_tolerance: float = 4.0) -> str:
    ...
    word_list = sorted(word_list, key=lambda w: (round(w["top"] / y_tolerance) * y_tolerance, w["x0"]))
...
def _extract_two_column(page, words: list[dict]) -> str:
    ...
    if split_x is None:
        ...
        logger.debug("  [layout] Gap detection failed — falling back to midpoint split.")
...
def extract_text_docx(file_path: str) -> tuple[str, str]:
    ...
    try:
        ...
        for textbox in body.iter(qn("w:txbxContent")):
            for child_para in textbox.iter(qn("w:p")):
                texts = [
                    node.text
                    for node in child_para.iter(qn("w:t"))
                    if node.text
                ]
                if texts:
    ...
    full_text = "\n".join(parts)
    logger.info(
        f"  [docx] Extracted {len(full_text)} chars from '{basename}'"
    )
...
def ocr_fallback(file_path: str) -> tuple[str, str]:
    ...
    try:
        ...
        logger.info(
            f"  [ocr] Extracted {len(full_text)} chars from '{basename}'"
        )
        return full_text, "ocr"

    except Exception as e:
        logger.error(
            f"  [ocr_failed] OCR failed for '{basename}': {e}"
        )
        return "", "failed"
...
def extract_sections(text: str, debug: bool = False) -> dict[str, str]:
    ...
    for raw_line in text.splitlines():
        ...
        if detected is not None:
            ...
            if detected in seen_sections:
                ...
                last_occurrence = len(transition_log) - 1 - transition_log[::-1].index(detected)
    ...
    for sec, score in confidence.items():
        if score < 0.5:
            ...
            logger.warning(msg)
            if _debug:
                print(f"[DEBUG] EMPTY SECTION WARNING: '{sec}' — "
